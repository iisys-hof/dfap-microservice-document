...
package de.iisys.duramentum.document.handler.folderHandler;

import com.hierynomus.msfscc.fileinformation.FileIdBothDirectoryInformation;
import com.hierynomus.smbj.share.DiskShare;
import de.iisys.duramentum.document.config.Configuration;
import de.iisys.duramentum.document.handler.SMB;
import de.iisys.duramentum.document.handler.strategy.SearchPattern;
import de.iisys.duramentum.document.handler.strategy.SearchWithIgnoreList;
import de.iisys.duramentum.document.handler.strategy.SearchWithMachineAndOrder;
import de.iisys.duramentum.document.handler.strategy.SearchWithTool;
...
import static de.iisys.duramentum.document.config.Configuration.getIgnoredResources;

/**
 * Specific FolderHandler for SMB shares. Uses SMB service which can be configured with config.xml
 */
...
public class FolderHandlerForSMB implements FolderHandler {
    ...
    /**
     * Returns a folder (and files) of an SMB share with a specific path or ID
     *
     * @param id String with folder ID. ID can start with "/" but it doesn't need to. The "/" will be removed. ID is the absolut path, beginning after SMB share
     *
    ...
    /**
     * Returns all folders (and files) of an SMB share
     *
    ...
    /**
     * Returns a {@link DocumentFolder} stucture for a given Machine ID and Tool ID or String which is found in the SMB share
     *
    ...
    public DocumentFolder getFolderByMachineAndTool(String machine, String tool) {
        ...
        getMachineAndTool(x, folder, new SearchWithMachineAndOrder(machine, tool), true);
    ...
    public DocumentFolder getFolderByTool(String tool) {
        ...
        DocumentFolder folder = getRootTree();
        DocumentFolder x = new DocumentFolder();
        getMachineAndTool(x, folder, new SearchWithTool(tool), true);

        return x;
    }

    @Override
    public DocumentFolder getFolderByToolInPath(String tool, String path) {
        DocumentFolder folder = getFolderTree(path);
        DocumentFolder x = new DocumentFolder();
        System.out.println("DD " +folder);
        getMachineAndTool(x, folder, new SearchWithTool(tool), true);

        return x;
    }

    private void getMachineAndTool(DocumentFolder x, DocumentFolder folder, SearchPattern sp, boolean first){
        if (first) {
            for(DocumentFile file : folder.getDocumentList()){
        ...
        for (DocumentFolder f : folder.getFolderList()) {
            ...
            getMachineAndTool(x, f, sp, false);
        }
    }
